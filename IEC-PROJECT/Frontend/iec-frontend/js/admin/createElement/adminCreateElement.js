import { apiApproveFlyer, apiDeclineFlyer, apiDelete } from "../../api/api.js";
...
export function createFlyerElems(flyerData, listId) {
    ...
    if (flyerData.record.status === "approved") {
        ...
        approvedBtn.id = "approved-btn";
        approvedBtn.className = "approved-btn";
        ...
        approvedBtn.addEventListener("click", async () => {
            ...
            if (result.success === true) {
                alert(result.message || "Flyer approved successfully!");
                window.location.reload();
            } else {
                alert(result.message || "Failed to approve flyer.");
            }
        });
        ...
        actionBox.appendChild(updateBtn);
        actionBox.appendChild(approvedBtn);
